...
[![](data/NFTPO.png){fig-align="center" width="100%"}](https://northfloridatpo.com/){target="_blank"}
...
The goal is to have zero injuries or fatalities resulting from crashes. The graph below shows the total number of impaired driving crashes in the 4 North Florida counties by year.
...
TTTR was obtained from the National Performance Management Research Data Set (NPMRDS), AADT from Highway Performance Monitoring System (HPMS), and occupancy factors provided by the FHWA. The goal is to maintain a TTTR ratio of under 2.0. For the North Florida TPO, the observed TTTR was 1.62 in 2024. The table below shows the TTTR on major freight corridors within the TPO boundary.
...
```{r, message=FALSE}
# Load your GeoJSON file
geojson_data_unfiltered <- st_read("data/Sidewalks_and_Gaps_NF.geojson", quiet = TRUE)
...
The Bridge Conditions Map and Bridge Conditions Table are presented below.
...
df_transit_safety <- read.csv("data/Transit_Safety_Targets.csv", check.names=FALSE, header=TRUE) 

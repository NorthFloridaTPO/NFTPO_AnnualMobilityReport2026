...
The Jacksonville Transportation Authority tracks transit ridership metrics. Ridership levels have declined since 2020, but are on the rise in recent years driven mostly by transit ridership increases in Duval County. The Jacksonville Transportation Authority operates and maintains the St. Johns River Ferry, ReadiRide (on-call transportation service), Clay Flex, St. Johns Express, Clay Express, and Nassau Express. On-time performance was tracked only for the St. Johns River Ferry during 2025 and this came in at 98.5%.
...
The maps below show the critical (maximum) Level of Travel Time Reliability of the four time periods (AM Peak, Mid Day, PM Peak, and Weekends) in 2025. They are separated out by route signage type: Interstate roadways, State roadways, and neither State nor Interstate roadways.

Source: [Level of Travel Time Reliability](https://npmrds.ritis.org/){target="_blank"}

::: {.panel-tabset .nav-pills}
### LOTTR Map: All Roadways
...
More detailed analysis of the Level of Travel Time Reliability was performed on the most congested corridors within the region. The table below summarizes the 2025 Level of Travel Time Reliability values of 1.6 or greater for the region and the corresponding critical segment. These values are taken for the PM peak period. This methodology holds for the rest of the tables presented for other types of roadways.
...
::: {.panel-tabset .nav-pills}
### LOTTR Map: Interstate Roadways Only

```{r}

# Load your GeoJSON file
geojson_data <- st_read("data/Florida_TMC_LOTTR2025_INTERSTATE.geojson", quiet = TRUE)  %>% 
  st_transform(crs = 4326)
...
More detailed analysis of the Level of Travel Time Reliability was performed on the most congested corridors within the region. The table below summarizes the 2025 Level of Travel Time Reliability values of 1.6 or greater for the region and the corresponding critical segment. These values are taken for the PM peak period.
...
### LOTTR Map: State Roadways Only

```{r}

# Load your GeoJSON file
geojson_data <- st_read("data/Florida_TMC_LOTTR2025_STATE.geojson", quiet = TRUE)  %>% 
  st_transform(crs = 4326)
...
::: {.panel-tabset .nav-pills}
### LOTTR Map: Non-State Roadways Only

```{r}

# Load your GeoJSON file
geojson_data <- st_read("data/Florida_TMC_LOTTR2025_NEITHER.geojson", quiet = TRUE)  %>% 
  st_transform(crs = 4326)
...
hchart(df_cost_fatal, "column", hcaes(x = year, y = cost, group = county)) %>%
  ...
  hc_subtitle(text = 'Source: <a href="https://signal4analytics.com/" target="_blank"><b>Signal Four Analytics</b></a> and <a href="https://fdotwww.blob.core.windows.net/sitefinity/docs/default-source/roadway/fdm/2026/2026fdm122varexcept.pdf?sfvrsn=c229234b_3" target="_blank"><b>Florida Department of Transportation Design Manual</b></a>') %>%
  hc_xAxis(title = list(text = "Year")) %>%
  hc_tooltip(shared = TRUE, pointFormat = "<span style='color:{series.color}'>{series.name}</span>: <b>{point.y}</b><br/>") %>%
  hc_legend(enabled = TRUE) %>%
  hc_add_theme(hc_theme_smpl()) %>%

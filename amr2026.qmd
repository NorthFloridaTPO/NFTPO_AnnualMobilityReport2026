...
::: {layout="[ [1,1] ]"}
```{r}
df_bike_ped <- read.csv("data/NF_Bike_Ped_fatalcrashes_20_25.csv")
...
hchart(df_commute, "column", hcaes(x = County, y = Mins)) %>%
  ...
  hc_title(text = "2023 Average Commute Times by County") %>%
...
hchart(df_inc_all, "bar", hcaes(x = FY24_Qtr, y = Mins, group = Category)) %>%
  ...
  hc_title(text = "2024 Incident Clearance Duration") %>%
...
hchart(df_duration, "column", hcaes(x = Category, y = Mins, group = FY24_Qtr)) %>%
  ...
  hc_title(text = "2024 Open Roads and Roadway Clearance Durations by Quarter") %>%
  hc_subtitle(text = 'Source: <a href="https://sunguide.info/" target="_blank"><b>Florida Department of Transportation SunGuide</b></a>') %>%
  hc_yAxis(title = list(text = "Duration (Mins)")) %>%
...
hchart(df_events, "pie", hcaes(x = Event, y = Total)) %>%
  ...
  hc_title(text = "2024 Incident Event Totals", align = 'left', style = list(fontFamily = 'Roboto Condensed', color = '#333333', fill = '#333333', fontSize = '18px', fontWeight = 'bold')) %>%
